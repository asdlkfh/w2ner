...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data_batch in enumerate(data_loader):
            data_batch = [data.cuda() for data in data_batch[:-1]] #将数据放入cuda中

            bert_inputs, grid_labels, grid_mask2d, pieces2word, dist_inputs, sent_length = data_batch

            outputs = model(bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length) #模型输出的是每句话中每个字两两之间的位置的向量，即矩阵中每个坐标的向量
            # outputs = self.model(bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length,sent_length.max().unsqueeze(0))
            grid_mask2d = grid_mask2d.clone()
            loss = self.criterion(outputs[grid_mask2d], grid_labels[grid_mask2d])  #将outputs矩阵中的非padding的位置取出来，总共12367个，然后每个位置为10维的向量即10个类别
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), config.clip_grad_norm)  #梯度裁剪，让梯度都小于1，解决梯度爆炸
            self.optimizer.step()
...
if __name__ == '__main__':
    ...
    config = config.Config(args) #将传入的json里的参数封装到config里，通过封装可以在log日志里全部打印出来

    logger = utils.get_logger(config.dataset) #实例化logging日志类
    logger.info(config)
    ...
    datasets, ori_data = data_loader.load_data_bert(config)  #输出构造的数据和原始数据(训练集，验证集，测试集)

    train_loader, dev_loader, test_loader = (
        DataLoader(dataset=dataset,
                   batch_size=config.batch_size,
                   collate_fn=data_loader.collate_fn,
                   shuffle=i == 0,                           #i等于0表示训练集
                   num_workers=4,
                   drop_last=i == 0)  
